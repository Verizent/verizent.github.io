    ...
    <section className="mb-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {projects.map((project, index) => (
          <div
            key={index}
            className="backdrop-blur-xl glass-card glass-card-hover rounded-2xl p-5 shadow-xl hover:shadow-2xl hover:bg-white/40 transition-all duration-300 hover:scale-[1.02]"
          >
            <h3 className="text-lg font-semibold text-gray-800 dark:text-slate-100 mb-2">{project.name}</h3>
            <p className="text-gray-700 dark:text-slate-300 mb-4">{project.description}</p>
            {project.github && <a
              href={project.github}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex px-4 py-2 backdrop-blur-lg bg-white/40 hover:bg-white/60 text-gray-700 hover:text-gray-800 dark:text-slate-100 border border-white/30 hover:border-white/40 text-sm font-medium rounded-full transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 mr-2"
            >
            ...
            {project.link && <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex px-4 py-2 backdrop-blur-lg bg-white/40 hover:bg-white/60 text-gray-700 hover:text-gray-800 dark:text-slate-100 border border-white/30 hover:border-white/40 text-sm font-medium rounded-full transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
            >
              View Website
            </a>}
          </div>
